...
## OBTER O INPUT
##############################################################################################################################################################################################################################
diretoria = os.listdir("sequencias")
teste = ""
ground = []
...
with open("data.csv","w") as ficheiro:
    ...
    for i in diretoria:
        if(i[0]!="."):
            if("X" in i): ground.append([i,0])
            else: ground.append([i,1])
            # obter as imagens desta sequência
            diretoria_aux = os.listdir("sequencias/" + i)
            teste = []
            for j in diretoria_aux:
                if(j[0]!="."):
                    teste.append("sequencias/" + i + "/" + j)
            for j in range(num_testes):
                # if(randint(0,1)==0): shuffle(teste)
                if("X" in i): ficheiro.write(",".join(teste) + ",0,2\n")
                else: ficheiro.write(",".join(teste) + ",1,2\n")
##############################################################################################################################################################################################################################
...
indice = 0
'''
for i in range(0,len(predicted_test),1):
    if((i%num_testes)!=0):
        if(predicted_test[i]==0): aux_0 += 1
        else: aux_1 += 1
    else:
        if(aux_0>aux_1): resposta_rede.append([ground[indice][0],0])
        else: resposta_rede.append([ground[indice][0],1])
        indice += 1
        aux_0 = 0
        aux_1 = 0
        if(predicted_test[i]==0): aux_0 += 1
...
for i in range(0,len(ground),1):
    if(ground[i][1]==predicted_test[i]): acertos += 1

print("Taxa de acerto: " + str((float(acertos)/len(ground))*100) + "%")
print(ground)
