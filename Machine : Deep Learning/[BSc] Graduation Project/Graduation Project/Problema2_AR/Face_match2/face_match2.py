...
for i in base_dados:
    if(i[0]!="."):
        ...
        for j in base_dados:
            if((input_image.split("-")[0])!=(j.split("-")[0])): continue # géneros diferentes
            lista_aux = ["grupos_BD/" + i,"grupos_BD/" + j]
            # shuffle(lista_aux)
            testes.append(["input/" + input_image, lista_aux[0], lista_aux[1]])
...
with open("data.csv","w") as ficheiro:
    ficheiro.write(base)
    for i in testes:
        ficheiro.write(i[0] + "," + i[1] + "," + i[2] + ",0,2\n")
##############################################################################################################################################################################################################################
...
for index, elemento in enumerate(test_samples):
    ...
    individuos[(individuos_index.index(elemento[2].split("/")[1]))][2] += 1
    individuos[(individuos_index.index(elemento[2].split("/")[1]))][3].append(int(predicted_test[index]))
    individuos[(individuos_index.index(elemento[2].split("/")[1]))][1] = float(sum(individuos[(individuos_index.index(elemento[2].split("/")[1]))][3]))/individuos[(individuos_index.index(elemento[2].split("/")[1]))][2] 
...
with open("face_match2_ranking.txt","a+") as ficheiro:
    ficheiro.write(str(pos) + " " + str(iguais-1) + "\n")

# guardar o ranking num ficheiro específico
with open("face_match2_top.txt","a+") as ficheiro:
    ficheiro.write("-------------------------\n")
