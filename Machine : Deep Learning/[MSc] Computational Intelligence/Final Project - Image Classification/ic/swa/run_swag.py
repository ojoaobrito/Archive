...
class swish_layer(nn.Module):

    def __init__(self):
        super(swish_layer,self).__init__()
...
g = make_dot(y.mean(),params=dict(model.named_parameters())).render("vgg", format="png")
print(model)
'''
if(args.act_fun=="mish" or args.act_fun=="swish"):

    if(args.act_fun=="mish"): act = mish_layer()
    if(args.act_fun=="swish"): act = swish_layer()

    model.classifier[5] = act
    
print(model)
'''
y = model(Variable(torch.ones(128, 3, 32, 32)))
g = make_dot(y.mean(),params=dict(model.named_parameters())).render("vgg_mish", format="png")
'''
