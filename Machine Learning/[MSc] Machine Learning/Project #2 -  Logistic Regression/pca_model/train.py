...
def train(lr, tot_iter, training_file, fold):
    ...
    ####################################################################################
    if(not os.path.exists("learned_thetas_pca.csv")): mode = "w"
    else: mode = "a"

    with open("learned_thetas_pca.csv",mode) as file:
...
if __name__ == "__main__":
    ...
    if(os.path.exists("learned_thetas_pca.csv")):
        os.remove("learned_thetas_pca.csv")
    ...
    for i in range(K):
        ...
        results = train(LEARNING_RATE,TOT_ITERATIONS,"data/voice_training_fold_" + str(i+1) + "_pca.csv",i+1)
    ...
    plt.title("PCA Gradient Descent Loss ({:.4f} ± {:.4f}".format(mean,standard_deviation) + ")")
    plt.grid()
    ...
    plt.title("PCA Tensorflow Loss ({:.4f} ± {:.4f}".format(mean,standard_deviation) + ")")
    plt.grid()
