...
for i in diretoria:
    ...
    print("REDIMENSIONAR - TREINO - AFASTA (" + str(aux) + "/" + str(contador) + ")")
    aux += 1
    for j in sequencia:
...
print("REDIMENSIONAR - TREINO - AFASTA (DONE!)")
...
for i in diretoria:
    ...
    sequencia = os.listdir(path + "/" + i)
    sequencia = natsort.natsorted(sequencia,reverse=False)
    print("REDIMENSIONAR - TREINO - APROXIMA (" + str(aux) + "/" + str(contador) + ")")
    aux += 1
    for j in sequencia:
        if(j[0]=="." or j=="1escolha.txt"): continue
        try:
...
print("REDIMENSIONAR - TREINO - APROXIMA (DONE!)")
...
for i in diretoria:
    ...
    sequencia = os.listdir(path + "/" + i)
    sequencia = natsort.natsorted(sequencia,reverse=False)
    print("REDIMENSIONAR - TREINO - LATERAL (" + str(aux) + "/" + str(contador) + ")")
    aux += 1
    for j in sequencia:
        if(j[0]=="." or j=="1escolha.txt"): continue
        try:
...
print("REDIMENSIONAR - TREINO - LATERAL (DONE!)")
print("REDIMENSIONAR - TREINO (DONE!)")
