...
STOP_CRITERIUM = [60] #[10, 0.05]  #1 value: TOT_epochs; 2 values: last N iterations minimum percentual improvement
...
if np.size(SHOW_WORST_CASES) > 0:
    for i in range(SHOW_WORST_CASES):
        ...
        diretoria_nome = diretoria_nome.split("-")[0] + "/" + diretoria_nome.split("-")[1]

        # montar a imagem final
        diretoria = os.listdir("grupos_redimensionados/teste/" + diretoria_nome)

        imagens = []
        for i in diretoria:
            if(i!="1escolha.txt"): imagens.append(i)
        
        for i in range(len(imagens)): imagens[i] = "grupos_redimensionados/teste/" + diretoria_nome + "/" + imagens[i]
